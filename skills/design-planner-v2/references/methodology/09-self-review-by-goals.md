...
> 输入：所有已落 schema 的 craft 改动 + 当前整屏 schema + 截图。
> 输出：工作记忆里逐条 successCriteria 对账结果 + task notes 一句话留痕；不达回头改方案重做（最多 3 轮）。
>
> 任务目的：用具体可视判据 + 截图客观对账,杜绝抽象评分蒙混。
>
> ⚠️ 文中若提到「写 craft md 末尾自审段 / 写 self-review md」，按 `../README.md` 阅读规则执行——逐条对账在工作记忆里跑，结果落 task notes（≤200 字）。
