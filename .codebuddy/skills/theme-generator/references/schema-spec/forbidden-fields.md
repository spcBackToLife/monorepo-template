...
- ❌ **写顶层 `themeConfig.intent / tokens / decorationRules / iconSpec / stateSpec`**（v1.0 已废弃，必须写 `themes[active].*` 内）
- ❌ **用废弃的 MCP action**：`theme/update_tokens / set_intent / set_decoration / switch_variant`（v1.0 已删，请用 `set_theme_tokens / set_theme_intent / set_theme_decoration / switch_theme`）
- ❌ token 值用裸数字/字符串而非 hex/HSL/CSS 标准值（MCP 自动包装；但要保证语义正确）
- ❌ colorSchemes[].overrides 写完整 token 集（只写"和 base 不同"的差异）
- ❌ tokens 中混入未定义的"私货"字段（如 `customColors.brand1`，请用 `tokens.custom.*`）
- ❌ decorationRules 内嵌 React 组件 / 函数 / 表达式（必须是纯数据）
- ❌ 在 intent.summary 里塞所有维度（不是 dump 字段，是 ≤30 字定性）
- ❌ 节点样式或 token 引用里用别名（如 `$token:bgPage`）—— resolveTokens 不做别名映射，找不到 token 原样输出。引用必须用真理名（`$token:background`）。详见 `token-naming.md`。
