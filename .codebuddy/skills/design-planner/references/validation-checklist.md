...
**规则**: index.md §5(状态矩阵) 中列出的每个非基准状态，必须在 §7(节点树) 中有对应的 DOM 结构。

**验证方法**:
```
遍历 §5 的每一行状态 → 问: "这个状态需要什么特殊UI？"
...
**规则**: §7 节点树中的每个节点，必须能在对应组件 `<name>.md` 中找到样式规格；
基础元素（input/button 等）则由 design-system.md 统管，组件 .md 中应有"引用基础组件 + 本地参数"说明。

**验证方法**:
```
逐行扫描 §7 的节点 → 在所属组件 .md 中查找 → 确认有完整的样式值
缺失的必须补写到对应组件 .md
```
...
1. **素材覆盖**:
   - 遍历 §7 所有 [素材:X] 标注 → 确认 materials/ 有对应 .md
...
3. **装饰覆盖**:
   - visual.md 装饰策略表的每个装饰 → §7 中必须有对应节点

## Step 4.5: 数据/事件完整性验证

1. 对照交互文档的事件列表 → §6 必须有对应的事件定义
2. §5 状态矩阵引用的每个变量 → §6 stateInit 中必须有定义
3. §6 事件流中引用的每个 effect.fetch → §6 数据源定义中必须有

## Step 4.6: 组件独立文档存在性验证（★ 新增）

**规则**: 每个 `_component.json` 必须有对应的 `<name>.visual.md + <name>.md`，
路径在 `design-plan/components/<name>/` 或 `design-plan/pages/<id>/components/<name>/` 任一处。

**验证方法**:
```
跑 validate.ts --registry $REGISTRY → 第 6 条规则会自动报告：
  ❌ 组件 X 缺独立文档：应在 design-plan/components/X/ 或 design-plan/pages/<id>/components/X/ 任一处
  ⚠️ 组件 X 缺 visual.md（结构文档已存在）
  ⚠️ 组件 X 在通用目录和页面专属目录两处都有文档，路径冲突
```

**对应每个组件节点**:
- 该节点的 `design.visualRef` 必须指向 `<name>.visual.md` 独立文档（不能指页面 visual.md 章节锚点）
- 该节点的 `design.ref` 必须指向 `<name>.md` 独立文档
