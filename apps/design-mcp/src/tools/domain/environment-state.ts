...
export function registerEnvironmentTools(server: McpServer): void {
  registerDomainTool(server, 'environment_state', '项目级环境变量（environmentStates）的 CRUD、预览、绑定', {
    ...
    add: {
      ...
      handler: async (p) => {
        const result = await apiClient.executeOperation(p.projectId, { type: 'addEnvironmentState', params: { name:p.name, label:p.label, values:p.values, defaultValue:p.defaultValue } });
    ...
    set_preview: {
      ...
      handler: async (p) => {
        const result = await apiClient.executeOperation(p.projectId, { type: 'setEnvironmentPreview', params: { variableName:p.variableName, value:p.value } });
        return { content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) }] };
      },
    },
    add_binding: {
      ...
      handler: async (p) => {
        const result = await apiClient.executeOperation(p.projectId, { type: 'addEnvironmentBinding', params: { nodeId:p.nodeId, binding:{ variableName:p.variableName, value:p.value, styles:p.styles, props:p.props, visible:p.visible } } });
        return { content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) }] };
      },
    },
  });
