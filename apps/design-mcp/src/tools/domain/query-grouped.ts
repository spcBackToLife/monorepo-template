...
export function registerQueryTools(server: McpServer): void {
  registerDomainTool(server, 'query', '项目信息查询、屏幕列表、屏幕 Schema 获取', {
    project_info: defineAction({
      ...
      handler: async (p) => {
        const prj = (await apiClient.getProject(p.projectId)) as DesignProject;
    ...
    screen_schema: defineAction({
      ...
      handler: async (p) => {
        const prj = (await apiClient.getProject(p.projectId)) as DesignProject & { screens: Array<{id:string;name:string;rootNode:unknown;backgroundColor?:string}> };
