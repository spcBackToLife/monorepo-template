...
> 版本：v2.7 · 2026-05-31（**新增 §0.1.12 Expression Language v1.0 立约：平台第二 DSL**）
...
> 改造进度：v2.0 product-analyst / v2.1 theme-generator / v2.2 expectedArtifacts 任务级机器对账 / v2.3 UpstreamChallenge 跨阶段回流协议 / v2.4 R-EVENTS-01 退役 + anyNodeHasEvents / v2.5 翻译契约 DAM + nodeHasEvent + minimal-debug styles / v2.6 NetworkPolicy 网络层平台一等公民 / **v2.7 Expression Language v1.0 立约**（参考 Google CEL 三阶段架构，把"运行时表达式"从 string 子细节升级为平台第二 DSL）。
...
#### 0.1.12 Expression Language v1.0 —— 平台第二 DSL（v2.7 新增 ★）

**问题背景**（与 v2.6 NetworkPolicy 同构但量级更大）：

Schema-First 体系里所有标记为 `Expression<T>` 的字段（`visibleWhen` / `repeat.expression` / `props.*` / `styles.*` / `state.set.value` / `condition.when` / `action.params.*` / `logic.if.when` / `logic.switch.value` / `logic.switch.case.match` 等等）一直被当作"刚好用 string 装的字符串"处理：

- `Expression<T> = string & __brand` ——类型层只是字符串
- Parser/Evaluator 各立白名单——硬编码在源码注释里
- SKILL 文档教 AI 写 `Date.now()` `/regex/.test()`——但 Parser 实际不支持
- 表达式抛错被 `try/catch return true` 静默吞——错误永远暴露不出来
- 编辑器是普通 string `<input>`——零提示零校验

**事实链**：v0.3 上线以来从未把"表达式"作为一等公民对待。直到 v2.5 5 条漏译修复后 demo 真正跑起来，撞出 3 个用户可见症状（PhoneError 不显示 / PolicyText 文案消失 / SubmitBtn 无反应），全部根因都是"AI 按 SKILL 教学写出表达式，但 runtime 不支持，且静默吞错"。

**这是平台 DSL 层面的失守**：表达式被当成"实现细节"而非"系统化子语言"。完整根因分析见根目录 `PLATFORM-ROOT-CAUSE-ANALYSIS.md`。

**解决方案**：参考 Google CEL（业界工业级标杆，K8s/Istio/Cloud IAM 在用）的设计哲学，把表达式做成「平台第二 DSL」。

**v1.0 立约的核心产物**（位于 `features/design-schema/src/expression-lang/`）：

| 产物 | 文件 | 角色 |
|------|------|------|
| **机读真相源** | `spec.json` | 单一权威——所有"什么语法/标识符/方法被支持"判断从此 import |
| **人读规约** | `EXPR-LANG-SPEC.md` | AI/SKILL/人类共享的学习文档 + 速查表 |
| **TS 接口** | `index.ts` | 类型化访问器（`isContextualIdentifier` / `getBuiltinFunction` / ...）|

通过 design-schema barrel 导出 `EXPR_LANG_SPEC` / `EXPR_LANG_VERSION` / 14 个便利访问器，供 Parser / Evaluator / Lint / Editor / SKILL 公共消费。

**设计哲学（CEL 工业级）**：
1. **非图灵完备** —— 故意阉割（无函数定义/循环/递归），换可静态分析 + 必定终止
2. **三阶段** —— Parse / Check / Eval。前两阶段在编辑期 / 落库期完成，运行时只 Eval
3. **Scope 显式声明** —— 哪些 identifier、哪些 method 可用全在 spec.json
4. **错误结构化** —— 每条错误带 `code` + `hint` + `suggestedFix`，AI 友好

**v1.0 关键能力（解决 demo 撞出的 5 个具体 bug）**：

| Bug | v1.0 是否覆盖 |
|---|---|
| `Date.now()` 不能跑 | ✅ `Date` 进 globals 白名单 + 推荐 `$.now()` |
| `/regex/.test()` 不能跑 | ✅ 正则字面量进 syntax + regex 实例方法白名单 |
| `globalView.x` 不能跑 | ✅ `globalView` 进 contextual identifier |
| `state.view.x.length` 实例方法 | ✅ string 实例方法白名单（length/slice/includes/...） |
| `state.set value` 含 ternary + 正则 | ✅ 完整支持，含可选链 `?.` + 空合并 `??` |

**与既有 8 类阶段角色的关系**：

| 角色 | 与 Expression Lang 关系 |
|---|---|
| product-analyst | 不写表达式（只写产品叙事） |
| theme-generator | 仅写少量 `$token:xxx` 类 string（不进 expression lang） |
| **interaction-designer** | ⭐ 主要用户。events.actions / visibleWhen / condition.when / state.set.value 全是表达式 |
| design-planner | 偶尔在 visualStates 的 props 表达式 / styles 表达式中触及 |
| design-executor | 不直接写表达式 |

**EXPR 改造路线图**（按"从根因杜绝"原则）：
...
|---|---|---|
| 起点 | demo 撞出"endpoint.timeout 平台缺字段" | demo 撞出"表达式语法 AI 写得跑不通" |
| 性质 | 一个字段缺失 | 一个 DSL 缺失 |
| 解法 | 加 `networkPolicy` 子结构 + 五层闭环 | 立 spec.json + 七大产物 + 三阶段架构 |
| 量级 | 半天 | 一周 |
| 沉淀位置 | §0.1.11 | §0.1.12 |

二者**机制完全同构**：都是把"约定层"提升为"平台一等公民契约"。
