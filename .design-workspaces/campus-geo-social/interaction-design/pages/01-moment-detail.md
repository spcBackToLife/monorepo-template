...
├── location-card/
│   ├── _component.json              (mini map + 地名 + 距离)
│   └── view-detail-btn.json     (查看地点详情)
├── passers-stat-btn.json        (路过统计入口)
├── action-bar/
│   ├── _component.json              (底部固定，含点赞/评论/分享/更多)
│   ├── like-btn.json
│   ├── comment-btn.json
│   ├── share-btn.json
│   └── more-btn.json
└── more-menu/
    ├── _component.json              (component: ActionSheet)
