...
export function getAvailableOperations(): OperationDescription[] {
  return [
    ...
    {
      type: 'addDomainState',
      description: 'Add a domain state variable on a screen or container node',
      category: 'domainState',
      params: [
        { name: 'ownerId', type: 'string', required: true, description: 'Screen ID or container node ID' },
        { name: 'ownerType', type: "'screen' | 'node'", required: true, description: 'Whether ownerId refers to a screen or a node' },
        { name: 'name', type: 'string', required: true, description: 'Variable name (unique within the owner scope)' },
        { name: 'label', type: 'string', required: true, description: 'Human-readable label' },
        { name: 'values', type: 'DomainStateValue[]', required: true, description: 'Allowed values with labels' },
        { name: 'defaultValue', type: 'string', required: true, description: 'Default value key' },
      ],
    },
    {
      type: 'removeDomainState',
      description: 'Remove a domain state variable from a screen or node',
      category: 'domainState',
      params: [
        { name: 'ownerId', type: 'string', required: true, description: 'Screen or node ID' },
        { name: 'ownerType', type: "'screen' | 'node'", required: true, description: 'Owner kind' },
        { name: 'variableName', type: 'string', required: true, description: 'Name of the variable to remove' },
      ],
    },
    {
      type: 'updateDomainState',
      description: 'Update domain state variable definition (label, values, defaultValue)',
      category: 'domainState',
      params: [
        { name: 'ownerId', type: 'string', required: true, description: 'Screen or node ID' },
        { name: 'ownerType', type: "'screen' | 'node'", required: true, description: 'Owner kind' },
        { name: 'variableName', type: 'string', required: true, description: 'Variable to update' },
        { name: 'patch', type: 'object', required: true, description: 'Optional label, values, defaultValue' },
      ],
    },
    {
      type: 'setDomainStatePreview',
      description: 'Set editor preview value (currentPreviewValue) for a domain state variable',
      category: 'domainState',
      params: [
        { name: 'ownerId', type: 'string', required: true, description: 'Screen or node ID' },
        { name: 'ownerType', type: "'screen' | 'node'", required: true, description: 'Owner kind' },
        { name: 'variableName', type: 'string', required: true, description: 'Variable name' },
        { name: 'value', type: 'string', required: true, description: 'Preview value to apply' },
      ],
    },
    {
      type: 'addDomainStateBinding',
      description: 'Add a domain state binding (variableName + value) to a node',
      category: 'domainState',
      params: [
        { name: 'nodeId', type: 'string', required: true, description: 'Target node ID' },
        {
          name: 'binding',
          type: 'DomainStateBinding',
          required: true,
          description: 'variableName, value, optional ownerNodeId, styles, props, visible, childrenVisibility, disabledEvents',
        },
      ],
    },
    {
      type: 'removeDomainStateBinding',
      description: 'Remove a domain state binding identified by variableName and value',
      category: 'domainState',
      params: [
        { name: 'nodeId', type: 'string', required: true, description: 'Target node ID' },
        { name: 'variableName', type: 'string', required: true, description: 'Domain variable name' },
        { name: 'value', type: 'string', required: true, description: 'Binding value key' },
      ],
    },
    {
      type: 'updateDomainStateBinding',
      description: 'Update overrides on an existing domain state binding',
      category: 'domainState',
      params: [
        { name: 'nodeId', type: 'string', required: true, description: 'Target node ID' },
        { name: 'variableName', type: 'string', required: true, description: 'Domain variable name' },
        { name: 'value', type: 'string', required: true, description: 'Binding value key' },
        { name: 'patch', type: 'object', required: true, description: 'Partial styles, props, visible, childrenVisibility, disabledEvents' },
      ],
    },
    ...
    {
      type: 'addDataSource',
      description: 'Add a data source to a screen (API or static lifecycle)',
      category: 'data',
      params: [
        { name: 'screenId', type: 'string', required: true, description: 'ID of the screen' },
        {
          ...
          type: 'object',
          required: true,
          description: 'Data source descriptor: id, name, lifecycle (api | static), optional description',
        },
      ],
    },
    {
      type: 'removeDataSource',
      description: 'Remove a data source from a screen',
      category: 'data',
      params: [
        { name: 'screenId', type: 'string', required: true, description: 'ID of the screen' },
        { name: 'dataSourceId', type: 'string', required: true, description: 'ID of the data source to remove' },
      ],
    },
    {
      type: 'updateDataSource',
      description: 'Update data source metadata (name and/or description)',
      category: 'data',
      params: [
        { name: 'screenId', type: 'string', required: true, description: 'ID of the screen' },
        { name: 'dataSourceId', type: 'string', required: true, description: 'ID of the data source' },
    ...
    },
    {
      ...
      description: 'Switch the active lifecycle phase for preview (API data sources only)',
      category: 'data',
      params: [
        { name: 'screenId', type: 'string', required: true, description: 'ID of the screen' },
        { name: 'dataSourceId', type: 'string', required: true, description: 'ID of the data source' },
        { name: 'phase', type: 'string', required: true, description: 'Phase name (e.g. loading, loaded, empty, error)' },
      ],
    },
    {
      type: 'addDataScenario',
      description: 'Add a mock data scenario to a data source',
      category: 'data',
      params: [
        { name: 'screenId', type: 'string', required: true, description: 'ID of the screen' },
        { name: 'dataSourceId', type: 'string', required: true, description: 'ID of the data source' },
        { name: 'scenario', type: 'object', required: true, description: 'Scenario with id, name, data, optional description/isDefault' },
      ],
    },
    {
      type: 'updateDataScenario',
      description: 'Update scenario content (data and/or name and/or description)',
      category: 'data',
      params: [
        { name: 'screenId', type: 'string', required: true, description: 'ID of the screen' },
        { name: 'dataSourceId', type: 'string', required: true, description: 'ID of the data source' },
        { name: 'scenarioId', type: 'string', required: true, description: 'ID of the scenario' },
        { name: 'data', type: 'object', required: false, description: 'Replacement mock data object' },
        { name: 'name', type: 'string', required: false, description: 'New scenario name' },
        { name: 'description', type: 'string', required: false, description: 'New description' },
      ],
    },
    {
      type: 'removeDataScenario',
      description: 'Remove a mock data scenario from a data source',
      category: 'data',
      params: [
        { name: 'screenId', type: 'string', required: true, description: 'ID of the screen' },
        { name: 'dataSourceId', type: 'string', required: true, description: 'ID of the data source' },
        { name: 'scenarioId', type: 'string', required: true, description: 'ID of the scenario to remove' },
      ],
    },
    {
      type: 'switchDataScenario',
      description: 'Set the active scenario for preview (use empty string to clear)',
      category: 'data',
      params: [
        { name: 'screenId', type: 'string', required: true, description: 'ID of the screen' },
        { name: 'dataSourceId', type: 'string', required: true, description: 'ID of the data source' },
        { name: 'scenarioId', type: 'string', required: true, description: 'ID of the scenario to preview, or ""' },
      ],
    },
    {
      type: 'bindData',
