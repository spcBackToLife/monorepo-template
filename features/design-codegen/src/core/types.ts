...
export interface PageIR {
  ...
  /** Inferred TypeScript interfaces from data shapes (stateInit + mock responses) */
  inferredTypes: InferredTypeIR[];
}

/** An inferred TypeScript interface definition */
export interface InferredTypeIR {
  name: string;                    // "Message"
  fields: { name: string; type: string; optional?: boolean; description?: string }[];
}
...
export interface HookSplit {
  ...
  handler?: HandlerIR;
  stateVars: HookStateDef[];       // variables this hook manages (BUG #2 fix)
...
export interface ServiceSplit {
  ...
  relatedTypes: string[];  // Type names used by these functions (BUG #7 fix)
}
...
/** A complete emit plan for one screen — all files, their paths, and full render data */
export interface ScreenEmitPlan {
  files: FileEmitPlan[];
}

export interface FileEmitPlan {
  /** Relative output path (e.g., "src/pages/Chat/index.tsx") */
  outputPath: string;
  /** EJS pattern template name, or null for raw content output */
  pattern: string | null;
  /** Complete data for template rendering (includes full import lists) */
