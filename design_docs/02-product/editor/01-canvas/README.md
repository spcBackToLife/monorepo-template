...
```
画布容器（CanvasContainer）是固定的，占编辑器中央区域。
视口容器（ViewportContainer）是画布中的一个"画框"，大小 = 当前设备尺寸。
...
```
切换视口（如 iPhone 15 → iPad Air）：

  1. ViewportContainer 的 width/height 更新
  2. Schema 不变（内容完全不动）
  3. 重新执行 zoomToFit → 视口居中、缩放适配
  4. 更新节点包围盒缓存
  5. 用户可以直观看到 UI 在新视口下的表现

视口切换不是"响应式适配"（那是代码层面的事），
而是"在不同尺寸的窗口中预览同一份 Schema"。
```
...
理由：
  · 设计师可能有意让元素超出视口（如 off-screen 动画起点）
  · 真实 CSS 也允许 overflow
  · 但可以提供"视觉提示"：超出视口的部分灰度降低 + 虚线边界

视口溢出检测（可选的辅助功能）：
  · 遍历所有直接子元素
  · 如果 left + width > viewport.width 或 top + height > viewport.height
  · 在组件树中标记 ⚠️
  · 画布上显示超出区域的警告色条
```
