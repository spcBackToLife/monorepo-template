...
| 0.3.5 | `Screen` 扩展 | `types/screen.ts` | 新增 `dataSources: DataSource[]`，标记旧 `dataSets` 为 `@deprecated` |
### 0.4 组件态增强
| # | 任务 | 文件 | 说明 |
|---|------|------|------|
| 0.4.1 | `ComponentState` 扩展 | `types/state.ts` | 新增 `childrenVisibility?: Record<string, boolean>` 和 `disabledEvents?: string[]` |
| 0.4.2 | 旧 `GlobalStateVariable` | `types/state.ts` | 标记 `@deprecated`，替换为 `DomainStateVariable` + `EnvironmentVariable` |
| 0.4.3 | 旧 `GlobalStateBinding` | `types/state.ts` | 标记 `@deprecated`，替换为 `DomainStateBinding` + `EnvironmentStateBinding` |
### 0.5 事件类型更新
| # | 任务 | 文件 | 说明 |
|---|------|------|------|
| 0.5.1 | 新增 `setDomainState` action | `types/event.ts` | `{ type: 'setDomainState', variableName: string, value: string }` |
| 0.5.2 | 新增 `setEnvironmentState` action | `types/event.ts` | `{ type: 'setEnvironmentState', variableName: string, value: string }` |
| 0.5.3 | 旧 `setGlobalState` | `types/event.ts` | 标记 `@deprecated`，运行时自动映射到 `setDomainState` |
| 0.5.4 | `EventCondition` 更新 | `types/event.ts` | `type` 从 `'globalState'` 改为 `'domainState' \| 'environmentState'` |
### 0.6 迁移工具
| # | 任务 | 文件 | 说明 |
|---|------|------|------|
| 0.6.1 | `migrateDataSetsToDataSources()` | `migrations/dataSource.ts` | 旧 `DataSet[]` → 一个 `static` DataSource 的 scenarios |
| 0.6.2 | `migrateGlobalStatesToDomainAndEnvironment()` | `migrations/state.ts` | 旧 `globalStates[]` → 按语义拆分到领域态/环境态 |
| 0.6.3 | `migrateGlobalStateBindings()` | `migrations/state.ts` | 旧 `globalStateBindings[]` → `domainStateBindings[]` |
| 0.6.4 | `migrateProject()` | `migrations/index.ts` | 统一入口，调用上述全部迁移，保证旧项目可加载 |
### 0.7 导出更新
| # | 任务 | 文件 | 说明 |
|---|------|------|------|
...
```
✓ 所有新类型定义完毕且导出
✓ 旧类型标记 @deprecated 但仍可用
✓ 迁移工具通过单元测试（旧数据 → 新结构无损转换）
...
| # | 任务 | 文件 | 说明 |
|---|------|------|------|
| 1.5.1 | `setGlobalState` 兼容层 | `operations/globalState.ts` | 保留旧操作入口，内部转发到 `setDomainStatePreview` |
| 1.5.2 | `addGlobalStateVariable` 兼容层 | `operations/globalState.ts` | 内部转发到 `addDomainState` 或 `addEnvironmentState`（根据命名推断） |
| 1.5.3 | 旧 DataSet 操作兼容层 | `operations/dataset.ts` | 保留 `addDataSet/updateDataSet/switchDataSet`，内部操作 DataSource 的 scenarios |
...
| 3.1.5 | 废弃 `activeRightTab` | stores/editor | 标记 `@deprecated`，保留一段时间后删除 |
...
| # | 任务 | 文件 | 说明 |
|---|------|------|------|
| 3.9.1 | 废弃 `PropertyPanel` | `panels/PropertyPanel/` | 由 RightPanel 完全替代 |
| 3.9.2 | 废弃 `RightTabType` | `stores/editor/` | 移除 `activeRightTab` 相关 |
| 3.9.3 | 更新 `focusRightPanelTab` 调用方 | 全局搜索 | BottomToolbar 等适配新 API |
| 3.9.4 | 废弃 `panels/tabs/` 已迁移文件 | `panels/tabs/` | StylesTab/PropsTab/StatesTab/CodeTab → 删除 |
**Phase 3 完成标志：**
```
✓ 右侧面板是统一滚动面板（无 Tab）
✓ 状态上下文栏显示五层状态控制
...
| # | 任务 | 文件 | 说明 |
|---|------|------|------|
| 4.5.1 | 旧 DataTab 废弃 | `panels/tabs/DataTab/` | 数据管理移至左侧 DataView |
| 4.5.2 | 旧 GlobalStateVariableManager 废弃 | StatesTab 内 | 环境态部分 → DataView/EnvironmentManager；领域态部分 → StateContextBar |
| 4.5.3 | 旧 PageList 废弃 | `panels/PageList/` | 由 PageView 替代 |
| 4.5.4 | 旧 NodeTree 废弃 | `panels/NodeTree/` | 由 ElementView 替代 |
**Phase 4 完成标志：**
```
✓ 左侧面板有三个视图可切换
✓ 页面视图显示跳转关系
...
| 6.1.4 | 旧 Datasets API 兼容 | datasets module | 旧端点内部转发到 DataSource 操作 |
### 6.2 项目迁移
| # | 任务 | 文件 | 说明 |
|---|------|------|------|
| 6.2.1 | 项目加载时自动迁移 | `projects/projects.service.ts` | 加载旧格式项目时调用 `migrateProject()` |
| 6.2.2 | 迁移版本标记 | project schema | `schemaVersion: number`，迁移后标记 |
### 6.3 Operation 路由扩展
| # | 任务 | 文件 | 说明 |
|---|------|------|------|
...
| # | 任务 | 文件 | 说明 |
|---|------|------|------|
| 7.5.1 | 旧 `list_datasets` 兼容 | dataset-tools | 标记 `@deprecated`，内部转发 |
| 7.5.2 | 旧 `set_global_state` 兼容 | global-state-tools | 标记 `@deprecated`，内部转发到 `set_domain_state_preview` |
**Phase 7 完成标志：**
```
...
| 包/模块 | 主要变化 | 涉及 Phase |
|---------|---------|-----------|
| `design-schema` | 新增 DomainState/Environment/DataSource 类型；扩展 ComponentNode/Screen/Project | P0 |
| `design-operations` | 新增 ~20 个操作（领域态/环境态/数据源）；旧操作兼容层 | P1 |
...
| `design-api` | 新 DataSource API；项目迁移；Operation 路由 | P6 |
| `design-mcp` | 领域态/环境态/数据源工具；旧工具兼容 | P7 |
...
| 旧数据迁移失败 | 旧项目打不开 | 迁移函数充分测试 + 旧字段 optional 保留 + 回退机制 |
| MCP 旧工具废弃影响 AI 使用 | AI 调用失败 | 兼容层至少保留一个版本周期 |
| 统一面板过长 | 用户找不到目标区域 | Section 折叠记忆 + 快捷键跳转 + 极简元素自适应精简 |
| 数据源模型 vs 数据集的概念跳跃 | 用户不理解「数据源」 | 静态数据源的体验与旧数据集完全一致；API 数据源是纯新增能力 |
...
| 为什么保留旧类型的兼容层？ | 渐进迁移 | 旧项目、旧 MCP 调用、旧 API 客户端都需要过渡期 |
