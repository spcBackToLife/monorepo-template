...
| **E.2** design-mcp：build + 重新连接 IDE | ✅ | E MCP | ✅ | E.1 | 2026-05-09 | `9734464` |
...
|------|---|
| 总子项 | 17（重构）+ 2（外部） |
| 已完成 | 17（P.0, P.1, A.1, A.2, A.3, B.1, B.2, C.1, C.2, D.1, D.2, D.3, D.4, D.5, E.1, E.2, F.1 schema 层） |
| 进行中 | F.1 浏览器端到端验证（待用户在 design_front + design-api 跑预览）/ D.6（v1 概念面板按 v2 重建，可与业务并行推进） |
| 阻塞中 | — |
| 最新 commit | `9734464` — D.4 + E.1 + D.5 三合一（design_front typecheck 312→0，monorepo lint 0 errors，pre-commit 通过） |
| 全 monorepo typecheck | ✅ 0 错（2026-05-09 F.1 完工后复测） |
| design-mcp dist | ✅ `dist/index.js` 168KB（5/8 18:41，E.1 + E.2 用户已 reload 生效，v2 工具签名验证通过） |
| 下一步候选 | ① **F.1 浏览器验证**（用户跑 design_front + design-api，确认 screenEnter 拉历史 + 输入受控 + send 发后用户气泡 + 1.2s AI 回复 + 清空 inputDraft 全链路）；② **F.2 收尾**（删 C.2 临时迁移层 / RFC 加 §9 落地结果 / AGENTS.md §8 红线 / 顶部"最后更新"日期）；③ **D.6**（v2 重写 Blueprint / PreviewBar / StatesTab，规模较大，可独立推进） |
...
| 2026-05-09 | D.5 关键产出归档（来自完工截图）— ① design_front 整包 312 个 v1 残留 typecheck 错误 → 0；② monorepo `pnpm -r typecheck` + `pnpm -r lint` 全 0 错；③ pre-commit hook 完全通过；④ 一次性删除 5 大块 v1 死代码：Blueprint / PreviewBar / StatesTab / 3 个 RightPanel v1 子组件 / `index.tsx.orig` 备份；⑤ 一次性脚本批量替换 90 处 v1 op 名（覆盖 29 个文件）；⑥ 严格遵守 §九「无双版本」红线：能删则删，删了的功能登记为 D.6 后续按 v2 重写。**遗留待办**：D.6（v2 重写 Blueprint / PreviewBar / StatesTab，按新设计恢复"PRD 全景"等能力）/ E.2（用户在 IDE 中 reload window 加载新 design-mcp dist）/ F.1 / F.2（业务验收 + 收尾） | AI 助手 |
| 2026-05-09 | E.2 进度核对 — `apps/design-mcp/dist/index.js` 168KB（5/8 18:41，E.1 commit `9734464` 已含 build 产物）；剩余唯一动作：用户在 IDE 中 **Reload Window** 让 MCP host 加载新 dist。Build/产物层面已就绪，待用户操作后状态翻 ✅ | AI 助手 |
| 2026-05-09 | F.1 启动前阻塞核查 — `mcp_get_tool_description` 返回的 `data_source` 工具仍含 `switch_phase`、`domain_state` 工具仍存在（旧 v1 描述），但 dist 文件 grep 已含 `registerStateTools` / `view_add` / `data_set_init` / `switch_mock_scenario`（v2 词汇），证明 **dist 已是 v2，但 MCP host 进程仍在跑旧版本**。必须用户在 IDE 里手动 **Reload Window** 重启 MCP 子进程，否则 F.1 所有 MCP 调用会走到旧 v1 工具无法验证 v2。F.1 暂停在工具发现阶段，待用户 reload 后继续 | AI 助手 |
| 2026-05-09 | E.2 完成 — 用户 IDE Reload Window 后 MCP host 重新加载 v2 dist，二次拉工具描述显示 `state` 工具（含 `view_add` / `data_set_init` / `global_view_add` 等 11 个 action）+ `data_source` 工具（含 `set_endpoint` / `add_mock_scenario` / `switch_mock_scenario` 等 11 个 v2 action），无 `switch_phase` / `domain_state` 残留；E.2 状态翻 ✅，与 E.1 同 commit `9734464` | AI 助手 |
| 2026-05-09 | F.1 完成 — 833478e8 项目的 Chat 屏幕（sc_bc334cb7178a4905991e6）按 RFC §F.1 验收要求纯 MCP 重建：① 删除旧 static `ds_chat_messages`，新建 api `chat-list`（GET /chat/list，mock 场景 sc-default 返 4 条种子）+ api `chat-send`（POST /chat/send，mock 场景 sc-success delay 1200ms 返 `{ userMessage, aiReply }`，body `{ text: '{{ params.text }}' }`）；② 屏幕 stateInit：`view.inputDraft = ''`、`data.messages = []`（空数组初值，等 chat-list onSuccess 写入）；③ rootNode 挂 screenEnter 事件 → `effect.fetch chat-list` → onSuccess: `state.set 'data.messages' = '{{ $last.data }}'`；④ 输入框容器 `nd_4255a8eab9...` 通过 `element.change_type` 从 div 改为 input，`element.set_bind path='view.inputDraft'` 完成受控双向，补 placeholder 与 input 视觉样式（border:none / outline:none / display:block / color:#fff）；⑤ 删芯片行 `nd_37dd466d...`（不在验收范围）；⑥ 气泡 row `nd_d2f4373e...` 通过 `element.set_repeat repeat='state.data.messages'` 列表化，row.justifyContent / 气泡 background+radius / p.color 全用表达式 `{{ item.role === 'user' ? ... : ... }}` 切，p.textContent 改 `{{ item.text }}`，原 props.children 清空；⑦ 发送按钮 `nd_24eccbbd...` 加 click 事件，condition `{{ state.view.inputDraft.length > 0 }}`，actions: `effect.fetch chat-send` (params: `{ text: '{{ state.view.inputDraft }}' }`) onSuccess 三动作链 append userMessage / append aiReply / 清空 inputDraft；⑧ 全程通过 MCP（element/style/event/state/data_source/component_prop 6 个工具，~20 次调用）完成，未直接编辑 schema 文件；⑨ 全 monorepo typecheck 复测 0 错。**仍待浏览器端到端验证**：打开 design_front + design-api 跑预览，确认 screenEnter 自动加载 4 条历史消息、输入框可打字、send 按下后用户气泡立即出现 + 1.2s 后 AI 回复出现、发送后输入框清空 | AI 助手 |
| 2026-05-09 | F.1 踩坑备忘（沉淀给后续 MCP 调用） — ① **参数命名不一致**：`element.remove` 用 `elementId` 而非 `nodeId`（其他 element action 都用 `nodeId`），需要警惕；② **data_source.add 必填字段超预期**：`id`（屏幕内唯一）、mock 场景必传 `id`/`name`/`statusCode`/`delay`/`responseBody`，且 mock 字段名是 `mockScenarios: [...]` 数组而非单个 `mockScenario`；endpoint.body 必须是 object（不能是字符串表达式），表达式只能用在 body 内的字段值上；③ **state.view_add 用 variable 包一层**：`{ variable: { name, defaultValue, label, ... } }` 而非平铺；④ **style.update 不接受 null**：要删属性必须用 `style.reset properties: [...]`，传 null 走 zod 校验失败；⑤ **builtin 函数当前没有 `$.now()`**：A.2 落地的 builtin 仅 `$.length / $.format / $.upper / $.lower / $.includes / $.first / $.last`，mock 里造唯一 id 不能用 now()，验收范围内用静态字符串可接受；⑥ **autoFetchOnEnter 与 screenEnter 事件互斥使用**：autoFetchOnEnter=true 只触发 fetch 但结果只在 `state.effects.<id>.data` 不会自动写到 `state.data.*`，要驱动列表 repeat 必须显式写 screenEnter 事件 + onSuccess: state.set —— 这与 RFC §F.1 验收文字一致 | AI 助手 |
| 2026-05-09 | F.1 浏览器验证首发 bug — **C.2 迁移层遗漏 v1 pipe fallback 语法** ⚠️。Welcome 屏幕 4 个 p/h1 节点的 `props.children` / `props.textContent` 含 v1 时代 `{{ data.slide.xxx \| "fallback" }}` 管道 fallback 语法，v2 Parser 只支持 `||` 短路运算符，遇到裸 `|` 字符在 tokenize 阶段直接抛 `ExpressionParseError: unexpected character \`\|\``，连锁导致 4 条 React NodeRenderer 红屏。**根因**：C.2 的 `v1-to-v2-state-model.ts` 迁移脚本按 RFC §4.2 字段重映射（domainStates → stateInit.view 等），但**没有扫字符串字段里的表达式子语法做语法升级**（v1 的 `{{ x \| default }}` → v2 的 `{{ x \|\| default }}`）。**临时修复**：通过 MCP `component_prop.update_props` 手工把 4 个节点的 children + textContent 字串里的 `\|` 替换为 `\|\|`（nd_3972176a / nd_e89656b7 / nd_589ed5e3 / nd_8e90d535）。**根因修复待办（升级到 F.2 收尾之前必做）**：给迁移脚本加一轮"递归扫字符串字段找 `{{ }}` 表达式，用正则把 v1 `\s\|\s` 替成 `\s\|\|\s` 后重写回去"，跑一次 migration 覆盖所有老项目 | AI 助手 |
| 2026-05-09 | F.1 浏览器验证二号 bug — **编辑画布模式下 screenEnter / autoFetchOnEnter 均不触发** ⚠️。用户打开 Chat 屏幕发现"页面啥数据也没有，请求 mock 也没有"。查 `features/design-engine/src/preview/PreviewRenderer.tsx:206-223`：`screenEnter` lifecycle useEffect 和 `autoFetchOnEnter` 遍历 `dataSources` 触发 `effect.fetch` 的逻辑**都只在 PreviewRenderer 里跑，SchemaRenderer（编辑画布）完全不跑**。所以 `state.data.messages` 初值是 `[]` 时，编辑画布列表 repeat 空数组 → 完全不渲染气泡，给设计师"啥也没加载"的错觉；实际上切到预览模式 screenEnter 会 fire → effect.fetch chat-list → mock 300ms → onSuccess state.set data.messages → 4 条气泡。**临时修复**：把 Chat 屏幕 `stateInit.data.messages` 从 `[]` 改成含 4 条种子对象的 JSON（走 state.data_set_init），编辑画布立即有内容；切预览时 screenEnter 会用 chat-list mock 的返回覆盖（内容一致，视觉等价）。**产品层待办（D.6 PreviewBar 重建时并入）**：加"编辑期 mock 数据注入"机制——编辑画布时给每个 api 数据源可选一条激活的 mock scenario，在 render 前把 scenario.responseBody 当成该 effect 的 data 塞到 editor dataContext 里（不进运行时 store，只给编辑画布的表达式求值用）。这样设计师编辑就能看到有意义的数据而不用手写 stateInit.data 初值 | AI 助手 |
| 2026-05-09 | F.1 浏览器验证三号 bug — **C.2 迁移漏『data.xxx 标识符加 state. 前缀』语法升级** ⚠️⚠️⚠️。用户反馈 Home 屏幕"能看到数据配置但是页面没展示"。查 Home schema 所有 p/h2/h3 节点的 `props.children` 都是 `{{data.user.displayName}}` / `{{data.hero.greeting}}` / `{{data.featureTiles[0].title}}` / `{{data.popularPrompts[0].title}}` 等 **v1 裸 `data` 标识符**。但 v2 Evaluator（`features/design-engine/src/expression/Evaluator.ts:41-52`）只认 `state / item / index / parent / $last / $` 6 个根 identifier，未知 identifier 返 `undefined`（不抛错但求值得空字符串）。所以 Home 所有数据驱动文案全部求值得 undefined → textContent 空白 → 页面"啥也看不到"。**根因**：C.2 的 v1-to-v2-state-model.ts 迁移脚本漏了『字符串字段表达式子语法升级』这一大类转换（同时也漏了之前发现的 v1 pipe fallback `\|` → `\|\|`）。**一次性修复**：新建 `apps/design-mcp/scripts/fix-v1-expressions.mjs` 脚本，遍历整个项目所有 screens → 所有 nodes → `props`/`styles`/`visibleWhen`/`repeat` 字段 → 对每个字符串值扫 `{{ ... }}` 段，按两条规则升级：① `upgradeFallbackPipe`：孤立 `\|`（前后都不是 `\|`）换成 `\|\|`；② `upgradeDataIdentifier`：裸 `data.` / `data[` 标识符（前边是边界字符如空格/运算符/括号/开头）加 `state.` 前缀，字符串字面量内的 `data.` 不动。脚本支持 dry-run 和 --apply，用 `POST /api/projects/.../operations/batch`  一次性提交 `componentProps.update` / `style.update` / `element.setVisibleWhen` / `element.setRepeat` op。对 833478e8 跑了一遍：扫 88 个节点 → 升级 17 个 field-group（Welcome 4 + Home 13）→ 二次 dry-run 0 残留。**产品层补救**：Home 原有 static dataSource `页面数据`（id=2008c68fe67140ce91b71）注入到 `state.data['页面数据']` 嵌套一层，与升级后的表达式 `state.data.user.displayName` 无法匹配——这是"static dataSource 注入策略 vs 表达式期望结构"的契约矛盾。选择 RFC §2.2 的 stateInit.data 路线：把 4 个顶层 key（user/hero/featureTiles/popularPrompts）直接写到 `screen.stateInit.data`（表达式无缝工作），删掉原 static dataSource。同理 Welcome 的 `slides[0]` 写到 `state.data.slide`（含 4 个字段：title/eyebrow/description/emptyBanner），第一屏文案立即显示；分页轮播交互由 `view.welcomePagerIndex` 驱动的逻辑后续单独重建（不影响本次 F.1 验证）。**根因修复待办**（F.2 收尾前必做）：把脚本里的 `upgradeFallbackPipe` + `upgradeDataIdentifier` 两条规则**内嵌到 C.2 的 v1-to-v2-state-model.ts** 迁移层，这样后续任何 v1 项目迁移上来都会自动走语法升级，不用再跑这种 ad-hoc 脚本 | AI 助手 |
