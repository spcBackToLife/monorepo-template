...
> 条件样式表达式只是「响应」机制——state 变化后样式跟随变化。
> 它**不能替代**「触发」机制——用户操作后改变 state 的 click/change 事件。
>
> **完整的交互链路 = 事件(触发) + 条件样式(响应)。两者缺一不可。**

```
正确实现 Tab 切换 = 以下全部:
  ✅ stateInit 声明 view.loginMode（默认值 'code'）
  ✅ code-tab click event → state.set(view.loginMode, 'code')      ← 触发！
  ✅ password-tab click event → state.set(view.loginMode, 'password') ← 触发！
  ✅ 条件样式表达式（响应 state 显示正确背景色/文字色）           ← 响应
  ✅ visibleWhen（响应 state 显隐对应面板）                        ← 响应

如果只写了条件样式没写 click event = state 永远是初始值 = UI 永远不变！
```

**教训**: 上次 Executor 写了完整的条件样式表达式和 visibleWhen，但完全遗漏了 click event。
结果：Tab 看起来有高亮（因为默认值是 'code' → 条件表达式在初始化时算对了），
但点击"密码"Tab 无反应——因为没有事件去改变 state。
