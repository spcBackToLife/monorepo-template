...
read_file: ../common/references/expression-language-cheatsheet.md   // ★ Expression Language v1.0 速查（spec.json 自动派生）
...
| `../common/references/expression-language-cheatsheet.md` ★ | Expression Language v1.0 速查（spec.json 自动派生：contextual / globals / builtins / instanceMethods / errorCodes / migrations） | Phase 1 列任务清单时必须加载；写 `condition.when` / `visibleWhen` / `repeat.expression` / `Action.value` 前必须查阅；ops 层会跑 lint 拒非法表达式 |
