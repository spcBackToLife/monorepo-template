...
> **完整规约 → [`expression-language-cheatsheet.md`](./expression-language-cheatsheet.md)**（从 `features/design-schema/src/expression-lang/spec.json` 自动派生，spec v1.0+ 实时同步）

5 行核心约定：

1. **必须 `{{ ... }}` 包裹**——裸字符串 = 字面量；`{{ state.view.x }}` ≠ `"state.view.x"`。
2. **作用域**含 `state` / `globalView` / `item` / `index` / `parent` / `$last` / `$`（builtin 命名空间）。
3. **全局**含 `Date` / `Math` / `Number` / `String` / `Boolean` / `JSON` / `Object` / `Array`（仅静态白名单方法）；`window` / `eval` / `Function` / `process` / `document` / `fetch` / `localStorage` 等**全部禁止**（E007）。
4. **实例方法**走类型白名单（`string` / `number` / `array` / `regex`）；不在 spec 的方法报 E004。
5. **错码**结构化：E001 syntax / E002 unknown ident / E003 forbidden global member / E004 forbidden instance method / E005 forbidden syntax / E006 type warn / E007 forbidden global——每条都带 `hint` + `suggestedFix` + `specRef`。

⚠️ **AI 落库门禁**：ops 层（event/element/style/data-source）写入前会跑 lint，违规直接返回 `{success:false, issues:[...]}`，不会"擦屁股"悄悄落库。

⚠️ **integrity 检查**：R-EXPR-01 是 `warning` 级，不会让"现在合法的 schema"红——spec 演进时旧表达式落入 deprecated 区时只提示迁移。
