...
## 十一、写作纪律：先认清"谁在什么场景下读这份文档"（红线）

**典型翻车现象**：写一份给 AI 执行用的 skill / prompt / 系统说明时，往里塞「v1 是怎样、v2 改了什么、迁移说明、历史背景、版本对照表、changelog」。**这些内容是写给开发者/维护者看的，不是写给执行时的 AI 看的**——AI 读 skill 是为了"现在该怎么做"，不需要知道"曾经怎么做"。

### 11.1 强制先回答三问，再动笔

写任何 skill / prompt / 文档前，先答：

1. **谁读这份文档？** —— AI 执行时？skill 维护者？产品经理？code reviewer？
2. **在什么场景下读？** —— 接到任务即刻执行？改 skill 前 review 历史？做版本迁移决策？
3. **读完要做什么？** —— 立刻调 MCP / 写代码？理解设计动机？追溯回归原因？

**三问答案不一致 → 拆成两份文档，不要混写**。

### 11.2 执行用文档的红线

写给 AI **执行时**读的文档（skill / agent prompt / 系统指令）：

- ❌ 禁止「v1 / v2 / 旧版 / 新版」对照表
- ❌ 禁止「继承自 X / 不再 X / 不再用 X」这种**指向历史的**否定句
- ❌ 禁止「迁移说明 / 翻译表 / 改动 changelog」
- ❌ 禁止「为什么这样设计」的设计动机段（除非动机直接影响执行决策）
- ✅ 只写：现在该做什么、怎么做、做到什么程度算完、什么情况禁止做、什么情况停下来问

**判别题**：把这段文字盖住"v1"两字、"旧版"两字、"以前"两字——读完还能照做吗？
- 答"能" → 这段是**纯执行指令**，留
- 答"不能" → 这段是**历史对照**，立刻删；如果信息有用，重写为正面陈述（"做 X"，不是"不再做 Y"）

### 11.3 维护用文档放别处

设计动机、版本演进、迁移决策、与旧版差异 —— 这些**有保留价值**的元信息放：

- `<skill>/CHANGELOG.md`（独立文件，AI 执行时不读）
- 提交信息 / PR 描述
- ADR（Architecture Decision Record）
- 项目根目录的 `docs/skill-evolution/*.md`

**不要**塞进 SKILL.md / agent prompt / 系统说明本体。

### 11.4 反例（真实发生过）

```
❌ design-planner-v2/SKILL.md 第一版：
   ## 0. v2 与 v1 的唯一区别
   | 维度 | v1（design-planner）| v2（本技能）|
   |---|---|---|
   | 产出形态 | md（过程）+ schema（结果） | **只 schema** |
   | ...
   后果：AI 读 SKILL 时被迫加载"v1 是怎样的"上下文,
        而 v2 的 AI 根本不需要也不应该知道 v1 存在。
        定位混乱:把"给开发者看的 changelog"误当"给 AI 看的指令"。

✅ 正确写法：
   ## 0. 这个 skill 干什么
   设计师工作流：看 schema → 想清楚 → 直接调 MCP 写 schema → 截图自审 → 移交。
   不写 md。结论即结构化 schema 字段。
```

### 11.5 自检触发词

写文档时如果你正在敲下面任意一个词组，立刻停下来跑 §11.1 三问：

- "v1 / v2 / 旧版 / 新版 / 上一版"
- "不再 / 不再用 / 已废弃 / 已删除"
- "继承自 / 来自 / 沿用"
- "改了什么 / 与 X 的区别 / 翻译表 / 迁移说明"
- "历史上 / 以前 / 早期 / 原本"

这些词在**执行用文档**里 99% 是定位混淆的红旗。

---

> 最后更新：2026-06-02（新增第十一章「写作纪律：先认清读者与场景」）
> 维护者：@pikun
